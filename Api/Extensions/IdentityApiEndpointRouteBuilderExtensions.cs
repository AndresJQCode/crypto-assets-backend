...
internal static class IdentityApiEndpointRouteBuilderExtensions
{
    ...
    /// </summary>
    private sealed class PasswordResetAttemptTracker
    {
        ...
        where TUser : class, new()
    {
        ...
        IOptionsMonitor<BearerTokenOptions> bearerTokenOptions = endpoints.ServiceProvider.GetRequiredService<IOptionsMonitor<BearerTokenOptions>>();
        IEmailSender<TUser> emailSender = endpoints.ServiceProvider.GetRequiredService<IEmailSender<TUser>>();
        ...
        routeGroup.MapPost("/register", async Task<Results<Ok, ValidationProblem>>
            (RegisterDto registration, HttpContext context, IServiceProvider sp) =>
        {
            ...
            TUser newUser = registration.Adapt<TUser>();
            ...
            IdentityResult result = await userManager.CreateAsync(newUser, registration.Password);
            ...
            await SendConfirmationEmailAsync(newUser, userManager, context, email);
            return TypedResults.Ok();
        });

        routeGroup.MapPost("/login", async Task<Results<Ok<AccessTokenResponse>, EmptyHttpResult, ProblemHttpResult>>
            (LoginRequest login, [FromQuery] bool? useCookies, [FromQuery] bool? useSessionCookies, IServiceProvider sp, IHttpContextAccessor httpContextAccessor) =>
        {
            ...
            SignInResult result = await signInManager.PasswordSignInAsync(login.Email, login.Password, isPersistent, lockoutOnFailure: true);
        ...
        routeGroup.MapPost("/refresh", async Task<Results<Ok<AccessTokenResponse>, UnauthorizedHttpResult, SignInHttpResult, ChallengeHttpResult>>
            (RefreshRequest refreshRequest, IServiceProvider sp) =>
        {
            SignInManager<TUser> signInManager = sp.GetRequiredService<SignInManager<TUser>>();
        ...
        routeGroup.MapGet("/confirmEmail", async Task<Results<ContentHttpResult, UnauthorizedHttpResult>>
            ([FromQuery] string userId, [FromQuery] string code, [FromQuery] string? changedEmail, IServiceProvider sp) =>
        {
            ...
            return TypedResults.Text("Gracias por confirmar tu email.");
        })
        .Add(endpointBuilder =>
        {
        ...
        routeGroup.MapPost("/resendConfirmationEmail", async Task<Ok>
            (ResendConfirmationEmailRequest resendRequest, HttpContext context, IServiceProvider sp) =>
        {
            var userManager = sp.GetRequiredService<UserManager<TUser>>();
        ...
        routeGroup.MapPost("/forgotPassword", async Task<Results<Ok, ValidationProblem>>
            (ForgotPasswordRequest resetRequest, IServiceProvider sp) =>
        {
            var userManager = sp.GetRequiredService<UserManager<TUser>>();
            var appSettings = sp.GetRequiredService<IOptionsMonitor<AppSettings>>();
            ...
            if (user is not null && await userManager.IsEmailConfirmedAsync(user))
            {
                ...
                {
                    var cacheKey = PasswordResetCacheKeys.GetTrackerKey(resetRequest.Email);
                    ...
                    if (tracker != null)
                    {
                        ...
                        if (timeSinceLastSent < requiredInterval)
                        {
                            ...
                            return CreateValidationProblem("TooManyRequests",
        ...
        routeGroup.MapPost("/resetPassword", async Task<Results<Ok, ValidationProblem>>
            (ResetPasswordRequest resetRequest, IServiceProvider sp) =>
        {
            var userManager = sp.GetRequiredService<UserManager<TUser>>();
        ...
        accountGroup.MapPost("/2fa", async Task<Results<Ok<TwoFactorResponse>, ValidationProblem, NotFound>>
            (ClaimsPrincipal claimsPrincipal, TwoFactorRequest tfaRequest, IServiceProvider sp) =>
        {
        ...
        accountGroup.MapGet("/info", async Task<Results<Ok<InfoResponse>, ValidationProblem, NotFound>>
            (ClaimsPrincipal claimsPrincipal, IServiceProvider sp) =>
        {
            var userManager = sp.GetRequiredService<UserManager<TUser>>();
            if (await userManager.GetUserAsync(claimsPrincipal) is not { } user)
            {
        ...
        accountGroup.MapPost("/info", async Task<Results<Ok<InfoResponse>, ValidationProblem, NotFound>>
            (ClaimsPrincipal claimsPrincipal, InfoRequest infoRequest, HttpContext context, IServiceProvider sp) =>
        {
            var userManager = sp.GetRequiredService<UserManager<TUser>>();
            if (await userManager.GetUserAsync(claimsPrincipal) is not { } user)
            {
