...
description: Enforces strict Domain-Driven Design and CQRS in .NET Core. Use when designing or implementing domain logic, commands, queries, repositories, minimal APIs, or when the user asks about DDD/CQRS architecture.
---
...
3. **Minimal APIs** - Only pattern allowed for endpoints
4. **Three-Project Architecture** - Domain / Infrastructure / Api (Api contains Application logic)
   > The `Api/` project contains both HTTP endpoints (`Api/Apis/`) and Application layer concerns
   > (`Api/Application/` — Commands, Queries, DTOs, Validators, Behaviors).
   > This is a deliberate design decision. The logical separation is maintained via folders.
...
### 2. APPLICATION LOGIC (inside Api/ project)

> **Project structure**: The Application layer lives inside `Api/Application/`.
> There is no separate Application project. This is intentional for simplicity.
...
├── Application/
│   ├── Commands/            # State-modifying operations (with handlers)
│   ├── Queries/             # Read-only operations (with handlers)
...
### 4. API ENDPOINTS (Api/Apis/)

> Endpoints live in `Api/Apis/` alongside the Application logic in `Api/Application/`.
    ...
    : IRequestHandler<CreateWarehouseCommand, Result<Guid>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Use warehouseRepository.UnitOfWork - NEVER inject IUnitOfWork directly
        await warehouseRepository.UnitOfWork.SaveEntitiesAsync(cancellationToken);
...
// Handler in Api/Application/ (application logic inside Api project)
...
- [ ] **IUnitOfWork NOT injected directly** - use `warehouseRepository.UnitOfWork` instead
...
8. **Injecting IUnitOfWork directly** → Use `warehouseRepository.UnitOfWork` instead
...
### ✅ ALWAYS Use warehouseRepository.UnitOfWork
    ...
    IWarehouseRepository warehouseRepository)
{
    ...
    {
        var warehouse = await warehouseRepository.GetById(request.Id, tracking: true, ct)
            ?? throw new NotFoundException("Warehouse not found");

        warehouse.Update(request.Name, request.Location);
        warehouseRepository.Update(warehouse);

        // ✅ CORRECT - Use warehouseRepository.UnitOfWork
